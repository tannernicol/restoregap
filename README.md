...
Install the v0.11.7 release, then run the isolated demo fixture. The demo needs
`sqlite3` because its fixture is a real SQLite database.

```console
$ curl -sSfLO https://raw.githubusercontent.com/tannernicol/restoregap/v0.11.7/scripts/install.sh
$ less install.sh
$ RESTOREGAP_VERSION=v0.11.7 sh install.sh
$ export PATH="$HOME/.local/bin:$PATH"   # use /usr/local/bin when installing as root
$ git clone --depth 1 --branch v0.11.7 https://github.com/tannernicol/restoregap.git
$ cd restoregap
$ demo/run.sh
```
